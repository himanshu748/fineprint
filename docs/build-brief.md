...
**Demo:** an existing application is blocked under the curated development-window interpretation; a new entry with reused components is checked separately; planning two submissions in one path exposes the official FAQ/rules discrepancy. The agent explains it with cited KB entries and keeps the unclear status.

**Landing direction:** on September 22 the recovered Claude session confirmed the user's replacement choice: a product-focused page without Three.js. A dated recorded answer, an inspectable source, the real FAQ/rules conflict and an engine-computed before/after example lead to the desk. The earlier 3D direction is preserved only as a historical record in `scroll-world.md`.

**Evaluation:** 42 authored boundary/applicability fixtures and a fixed-context model baseline. Do not claim independent accuracy, complete rule coverage, organizer approval or competitive outcomes from these checks.
